...
public class MonitoredMetamodelLoader extends GenericMonitoredResourceLoader {
	...
	protected IStatus preprocessResourceSet(final IProgressMonitor monitor) {
		...
		ecoreMetamodelDependency.getResource(getResourceSet(), true);
		subMon.worked(5);
		if (subMon.isCanceled()) {
			return Status.CANCEL_STATUS;
		}
		
		// Add URI mappings to rs
		Map<URI, URI> uriMap = getResourceSet().getURIConverter().getURIMap();
		for(ImportMappings mapping : moflonProperties.getImportMappings()) {
			uriMap.put(URI.createURI(mapping.getKey()), URI.createURI(mapping.getValue()));
		}
	...
	private final List<Resource> createResourcesForUserDefinedMetamodels(final IProgressMonitor monitor) {
		...
		final SubMonitor subMon = SubMonitor.convert(monitor, "Creating resources for user-defined metamodels",
				additionalDependencies.size());
		for (final AdditionalDependencies userDefinedMetamodel : additionalDependencies) {
			...
			resourcesToLoad.add(dependency.getResource(getResourceSet(), false));
			subMon.split(1);
		}
	...
	private final IStatus loadUserDefinedMetamodels(final List<Resource> resourcesToLoad,
		...
		final MultiStatus resourceLoadingStatus = new MultiStatus(WorkspaceHelper.getPluginId(getClass()), IStatus.OK,
				"Resource loading status", null);
		final SubMonitor subMon = SubMonitor.convert(monitor, "Loading user-defined metamodels",
				resourcesToLoad.size());
		for (Resource resource : resourcesToLoad) {
			try {
				resource.load(null);
			} catch (final IOException e) {
				resourceLoadingStatus.add(new Status(IStatus.ERROR, WorkspaceHelper.getPluginId(getClass()),
